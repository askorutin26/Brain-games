  ...
  const rightAnswer = (currentExpression) => {
    ...
    const numberArray=[];
     ...
     for (let i = 0; i < arrayLength; i+=1) {
       if (elementsArr[i]==='..'){dotsPosition = i;
       numberArray.push('..')}
       else {numberArray.push (Number(elementsArr[i]));} 
     }
     if (dotsPosition+2 < arrayLength-1){step=numberArray[dotsPosition+2]-numberArray[dotsPosition+1];
     return (String(numberArray[dotsPosition+1]-step))}
     else {step=numberArray[dotsPosition-1]-numberArray[dotsPosition-2];
     return (String(numberArray[dotsPosition-1]+step))};
